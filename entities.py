...
class Recording_Entity:
    def __init__(self, row):
        ...
        self.task = database.getTaskById(self.task_id)
        self.samples = database.getRecordSamples(self.recording_id)
        self.timestamps = self.generateTimeStamps()

    def generateTimeStamps(self):
        timestamps = []
        for sample in self.samples:
            normalTimestamp = sample.timestamp - self.timestamp_start
            sample.normalTimestamp = normalTimestamp
            timestamps.append(normalTimestamp/1000)#time in milisseconds (?)
        return timestamps

    # def distanceToFigure_Horizontal(self):
    #     leftEyeDistances = []
    #     rightEyeDistances = []
    #     normalizedPositions = utils.getNormalizedPositionDurationsAsJSON(self.task)
    #
    #     for sample in self.samples:
    #         normalTimestamp = (sample.timestamp - self.timestamp_start)/1000
    #         figurePos= utils.getFigurePositionAtTimestamp(normalizedPositions, normalTimestamp)
    #         leftNormalPos = sample.left_normal
    #         rightNormalPos = sample.right_normal
    #
    #         leftDistance = abs(float(leftNormalPos[0]) - figurePos['x'])
    #         rightDistance = abs(float(rightNormalPos[0]) - figurePos['x'])
    #
    ...
    def getPositions_HV(self):
        ...
        rightPos = []
        for sample in self.samples:
            leftPos.append(sample.left_normal)
            rightPos.append(sample.right_normal)
...
class Sample_Entity:
    def __init__(self, row):
        ...
        self.left_normal = row[3].replace('(', '').replace(')', '').split(',')
        self.right_normal = row[4].replace('(', '').replace(')', '').split(',')
...
class Task_Entity:
    ...
    def print(self):
        print('- id: ' + str(self.task_id),
              ', animation_blueprint: ' + self.animation_blueprint,
              ', parameter_type: ' + str(self.parameter_type),
              ', parameter_data: ' + str(self.parameter_data))

    def getFigurePositions_HV(self, timestampsInMs):
        positions = []
        normalizedPositions = self.getNormalizedPositionDurationsAsJSON()

        i=0
        for timestamp in timestampsInMs:
            try:
                if timestamp > normalizedPositions[i+1]['DurationSoFarInMs']:
                    i += 1
                [xPos, yPos] = normalizedPositions[i]['ToPoint'].split(',')
            except:
                [xPos, yPos] = normalizedPositions[i]['ToPoint'].split(',')
    ...
    def getNormalizedPositionDurationsAsJSON(task):
        taskJson = json.loads(task.animation_blueprint)
        return taskJson['NormalizedPositionDurations']