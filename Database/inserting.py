...
def insert_featuredRecords(conn, featuredRecords):
    sql = f"INSERT INTO featured_records(recording_id, task_id, group_id, subject_id, screening_id, features, is_valid) " \
    ...
    for record in featuredRecords:
        ...
        cursor.execute(sql, (recording_id, task_id, group_id, subject_id, screening_id, json.dumps(record['Features']), True))
    conn.commit()
